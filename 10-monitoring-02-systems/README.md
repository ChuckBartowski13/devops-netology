...
## Ответ
    ...
    Мониторинг аппаратной части проекта:

        утилизация процессора;
        утилизация RAM (любое ПО будет расходовать ОЗУ);
        свободное место на диске (а также кол-во inode - даже при свободном месте на диске можно потерять возможность создания новых файлов);
        доступность сетевого интерфейса, а также загрузку канала (так как взаимодействие с платформой осуществляется по протоколу http - нужно мониторить состояние сети).

    Мониторинг программной части проекта:

        коды ответа web-сервера (все, чтобы сформировать статистику по соотношению ошибок к общему кол-ву);
        состояние самого web-сервера (статус сервиса web-сервера на хосте);
        кол-во обращений к серверу (чтобы собрать пиковые значения и вовремя масштабировать систему).
...
CPUload average — показатель средней загрузки процессора за определенное время.

Нужно объяснить суть терминов и скорость реакции на исправление инцидентов:
...
Можно развернуть бесплатный Elastic Stack
...
В формуле не учитываются коды ответа 1хх и 3xx. Скорее всего неохваченная доля выполнения приходится на них. Корректнее было бы считать так: (summ_1xx_requests + summ_2xx_requests + summ_3xx_requests)/summ_all_requests
